...
describe('One-time checkout API', () => {
  ...
  it('returns a safe public checkout result without provider or customer details', async () => {
    const accessToken = await registerAndLogin();
    const providerCheckoutSessionId = `cs_${randomUUID()}`;
    queuedCheckoutSessions.push({
      id: providerCheckoutSessionId,
      paymentIntentId: `pi_${randomUUID()}`,
      url: 'https://checkout.stripe.test/safe-result',
    });
    await request(app.getHttpServer())
      .post('/stripe-connect/onboarding')
      .set('Authorization', `Bearer ${accessToken}`)
    ...
    const customerResponse = await request(app.getHttpServer())
      .post('/customers')
      .set('Authorization', `Bearer ${accessToken}`)
      .send({ name: 'Private Buyer', email: 'private-buyer@example.com' })
      .expect(201);
    const customer = customerResponse.body as CustomerResponse;
    ...
    await request(app.getHttpServer())
      .post('/payment-requests')
      .set('Authorization', `Bearer ${accessToken}`)
      .set('Idempotency-Key', 'safe-checkout-result')
      .send({
        customerId: customer.id,
        amount: 14_500,
    ...
    const response = await request(app.getHttpServer())
      .get(`/pay/checkout-result?session_id=${providerCheckoutSessionId}`)
      .expect(200);
  ...
  it('assigns USD when checkout creation omits currency', async () => {
    const accessToken = await registerAndLogin();
    queuedCheckoutSessions.push({
      id: `cs_${randomUUID()}`,
      paymentIntentId: `pi_${randomUUID()}`,
      url: 'https://checkout.stripe.test/usd-default',
    });
    await request(app.getHttpServer())
      .post('/stripe-connect/onboarding')
      .set('Authorization', `Bearer ${accessToken}`)
  ...
  it('rejects a non-USD currency before calling Stripe', async () => {
    const accessToken = await registerAndLogin();
    await request(app.getHttpServer())
      .post('/stripe-connect/onboarding')
      .set('Authorization', `Bearer ${accessToken}`)
    ...
    expect(response.body).toEqual({
      message: 'Relay supports USD only',
      error: 'Bad Request',
      statusCode: 400,
    });
  });
