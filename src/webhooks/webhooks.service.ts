...
export class WebhooksService {
  ...
  async process(payload: Buffer, signature: string) {
    ...
    const connection = await this.prisma.stripeConnection.findUniqueOrThrow({
      where: { providerAccountId: event.connectedAccountId },
      select: { workspaceId: true },
    });
    const workspaceId = connection.workspaceId;

    if (!this.isSupportedEvent(event)) {
      return this.recordUnhandledEvent(event, workspaceId);
    }
    ...
    try {
      await this.recordMatchedEvent(event, workspaceId, payment.id);
      void this.webhookDeliveries
